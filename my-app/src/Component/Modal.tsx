...
export default function Example(props: { name: any; email: any; id: any; getData: (arg0: { name: any; email: any; id: any; }) => void; }) {
  ...
  const idChangeHandler = (e: { target: { value: any; }; }) => {
    setId(e.target.value);
    userData.id = e.target.value;
  };

  const formOnChageHandler = (e: { preventDefault: () => void; }) => {
    e.preventDefault();
    props.getData(userData);
    handleClose();
  };
  const fromOnSubmitHandler = (e: { preventDefault: () => void; }) => {
    e.preventDefault();
  };
